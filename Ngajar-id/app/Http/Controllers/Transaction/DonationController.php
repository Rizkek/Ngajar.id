...
use App\Http\Resources\Finance\DonasiResource;
...
class DonationController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $total_donasi = Cache::remember('total_donasi', 300, function () {
                return Donation::whereIn('status', ['paid', 'settlement', 'capture'])->sum('jumlah');
            });

            $donatur_count = Cache::remember('donatur_count', 300, function () {
                return Donation::whereIn('status', ['paid', 'settlement', 'capture'])->count();
            });

            // Ambil 10 riwayat donasi terbaru dari database
            $riwayat_donasi = Donation::whereIn('status', ['paid', 'settlement', 'capture'])
                ->latest('tanggal')
                ->take(10)
                ->get();
            ...
            if ($riwayat_donasi->isEmpty()) {
                ...
                $riwayat_donasi = Donation::latest('tanggal')->take(10)->get();

                // Also get total of all donations
                if ($total_donasi == 0) {
                    $total_donasi = Donation::sum('jumlah');
                    $donatur_count = Donation::count();
                }
            }
        ...
        try {
            $stats = [
                'total_donations' => Cache::remember('total_donations_stat', 300, function () {
                    return Donation::whereIn('status', ['paid', 'settlement', 'capture', 'success'])->sum('jumlah');
                }),
                'total_donors' => Cache::remember('total_donors_stat', 300, function () {
                    return Donation::whereIn('status', ['paid', 'settlement', 'capture', 'success'])->distinct('email')->count('email');
                }),
                'total_transactions' => Cache::remember('total_transactions_stat', 300, function () {
                    return Donation::whereIn('status', ['paid', 'settlement', 'capture', 'success'])->count();
                }),
                'avg_donation' => Cache::remember('avg_donation_stat', 300, function () {
                    return Donation::whereIn('status', ['paid', 'settlement', 'capture', 'success'])->avg('jumlah');
                }),
            ];
    ...
    public function recent(Request $request)
    {
        try {
            ...
            $donations = Donation::whereIn('status', ['paid', 'settlement', 'capture', 'success'])
                ->latest('tanggal')
                ->take($limit)
                ->get();
    ...
    public function riwayat()
    {
        ...
        $query = Donation::whereIn('status', ['paid', 'settlement', 'capture']);

        // Debug/Dev: Jika kosong, tampilkan semua (biar user bisa lihat data tes yang masih pending)
        if ($query->count() === 0) {
            $riwayat_donasi = Donation::orderBy('tanggal', 'desc')->paginate(20);
        } else {
    ...
    public function store(Request $request)
    {
        try {
            ...
            $donasi = Donation::create([
                'nama' => ($validated['anonymous'] ?? false) ? 'Hamba Allah' : ($validated['nama'] ?? 'Hamba Allah'),
    ...
    public function webhook(Request $request)
    {
        try {
            ...
            $donasi = Donation::where('nomor_transaksi', $result['order_id'])->first();
    ...
    public function paymentFinish(Request $request)
    {
        ...
        $donasi = Donation::where('nomor_transaksi', $orderId)->first();
