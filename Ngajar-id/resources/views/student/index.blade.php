...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            @if($lastClass)
                <x-cards.card padding="p-0" borderColor="border-teal-100">
                    <x-slot name="header">
                        <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                            <x-icons.material name="history" class="text-teal-600" />
                            Lanjutkan Belajar
                        </h2>
                        <x-badges.status color="teal">Terakhir Diakses</x-badges.status>
                    </x-slot>

                    <div class="p-6 md:flex gap-6 items-center group">
                        <!-- Class Thumbnail -->
                        <div class="w-full md:w-48 h-32 bg-gray-200 rounded-xl shrink-0 relative overflow-hidden mb-4 md:mb-0">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-200 to-slate-300">
                                    <x-icons.material name="school" size="4xl" class="text-slate-400" />
                                </div>
                            @endif
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-4 mb-4 text-sm text-slate-600">
                                <span class="flex items-center gap-1">
                                    <x-icons.material name="person" size="sm" class="text-slate-400" />
                                    {{ $lastClass->pengajar->name }}
                                </span>
                                <span class="flex items-center gap-1">
                                    <x-icons.material name="menu_book" size="sm" class="text-slate-400" />
                                    12 Materi
                                </span>
                            </div>

                            <a href="{{ route('belajar.show', ['kelas_id' => $lastClass->kelas_id]) }}">
                                <x-buttons.primary color="teal">
                                    Lanjut Belajar
            ...
            @else
                ...
                <x-empty-state 
                    icon="rocket_launch" 
                    title="Mulai Petualangan Belajarmu!" 
                    description="Anda belum mengikuti kelas apapun. Yuk jelajahi katalog kelas kami dan temukan skill baru."
        ...
        <div class="space-y-6">
                ...
                <div data-aos="fade-left" class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @foreach($availableCategories as $cat)
                            <a href="{{ route('student.dashboard', ['kategori' => $cat]) }}"
            ...
            @if($myClasses->isNotEmpty())
                <div data-aos="fade-left" data-aos-delay="100">
                    ...
                    @if($myClasses->count() > 3)
                        <a href="{{ route('student.kelas') }}" class="block mt-3 text-center text-sm text-teal-600 hover:underline">
            ...
            <div data-aos="fade-left" data-aos-delay="200" class="flex items-center justify-between mb-2">
                ...
                <a href="{{ route('student.katalog', $selectedKategori ? ['kategori' => $selectedKategori] : []) }}"
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <form action="{{ route('student.katalog.join', $kelas->kelas_id) }}" method="POST">
                                @csrf
                                <x-buttons.secondary type="submit" size="sm" color="gray" class="bg-slate-100 border-none shadow-none text-slate-700">
                                    <x-icons.material name="add" size="sm" class="mr-1 -ml-1" /> Gabung
                                </x-buttons.secondary>
                            </form>
                        </div>
                    </div>
            ...
            @empty
                <x-empty-state 
                    ...
                    description="{{ $selectedKategori ? 'Tidak ada kelas ' . $selectedKategori . ' yang tersedia saat ini.' : 'Belum ada rekomendasi kelas tersedia.' }}"
                    class="p-6 border-none bg-slate-50"
                />
            @endforelse

            <!-- Quick Link to Modules -->
            <a href="{{ route('student.materi') }}" data-aos="fade-up"
