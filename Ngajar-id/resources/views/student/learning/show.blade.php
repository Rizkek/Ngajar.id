...
<body class="bg-gray-50 h-screen flex flex-col overflow-hidden">
    ...
    <header class="bg-gray-900 border-b border-gray-800 h-16 flex items-center justify-between px-6 shrink-0 z-20 shadow-md">
        <div class="flex items-center gap-4">
            <a href="{{ route('student.kelas') }}" class="text-gray-400 hover:text-teal-400 transition flex items-center gap-2 group">
        ...
        <div class="flex items-center gap-6">
            <div class="hidden md:block w-48 group">
                <x-progress-bar :percentage="$progress" color="teal" showLabel="true" textColor="text-gray-400" wrapperColor="bg-gray-800" glow="true" height="h-1.5" />
            </div>
            <!-- Profile Avatar -->
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex-1 flex flex-col bg-white overflow-hidden relative" x-data="{ activeTab: 'materi' }">
            ...
            <div class="h-20 bg-white border-t border-gray-200 shrink-0 px-6 flex items-center justify-between z-20">
                <div>
                    @if($prevMateri)
                        <x-buttons.secondary href="{{ route('belajar.show', ['kelas_id' => $kelas->kelas_id, 'materi_id' => $prevMateri->materi_id]) }}" class="border-none hover:bg-gray-50 hover:text-teal-600 px-4">
                            <x-icons.material name="arrow_back" size="sm" class="mr-2 -ml-1" />
                            <span class="hidden sm:inline">Sebelumnya</span>
                        </x-buttons.secondary>
                    @else
                        <x-buttons.secondary disabled class="border-none px-4 opacity-50 cursor-not-allowed">
                            <x-icons.material name="arrow_back" size="sm" class="mr-2 -ml-1" />
                            <span class="hidden sm:inline">Sebelumnya</span>
                        </x-buttons.secondary>
                    @endif
                ...
                <div>
                    @if($nextMateri)
                        <x-buttons.primary onclick="finishMateri('{{ route('belajar.complete', $activeMateri->materi_id) }}', '{{ route('belajar.show', ['kelas_id' => $kelas->kelas_id, 'materi_id' => $nextMateri->materi_id]) }}')" class="shadow-md hover:shadow-lg px-6 py-3">
                    ...
                    @else
                        <x-buttons.primary onclick="finishMateri('{{ route('belajar.complete', $activeMateri->materi_id) }}', '{{ route('student.kelas') }}?completed=true')" color="green" class="bg-gradient-to-r from-green-500 to-green-600 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 px-6 py-3">
                            <x-icons.material name="check_circle" size="sm" class="mr-2 -ml-1" />
