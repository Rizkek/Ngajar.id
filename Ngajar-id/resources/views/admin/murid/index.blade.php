...
@section('title', 'Kelola Murid - Admin')

@section('content')
    <div class="container-fluid px-4">
        <div class="mb-6 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 mb-2">Kelola Murid</h1>
                <p class="text-slate-600">Manajemen murid, token, dan akses platform</p>
            </div>
        </div>

        <!-- Filters & Search - Modern Design -->
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            <form method="GET" action="{{ route('admin.murid.index') }}" class="flex flex-wrap gap-4">
            ...
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black">{{ $murids->total() }}</h3>
                    </div>
            ...
            <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black">{{ \App\Models\User::murid()->where('status', 'aktif')->count() }}
                        </h3>
                    </div>
                    <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black">
                            {{ \App\Models\User::murid()->where('status', 'nonaktif')->count() }}
                        </h3>
                    </div>
                    <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <span class="font-medium text-slate-700 block">{{ $murid->name }}</span>
                                            @if($murid->is_beasiswa)
                                                <span
                                                    class="inline-flex items-center gap-1 text-[10px] font-bold bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full mt-0.5">
                                                    <span class="material-symbols-rounded text-[10px]">school</span> Beasiswa
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $murid->email }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-700">
                                        {{ $murid->kelas_ikuti_count }} Kelas
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">
                                        {{ number_format($murid->token?->jumlah ?? 0) }} Token
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
