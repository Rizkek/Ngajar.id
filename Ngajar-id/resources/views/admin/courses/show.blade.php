        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3">
                        @if($kelas->status !== 'aktif')
                            <form action="{{ route('admin.courses.updateStatus', $kelas->kelas_id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="status" value="aktif">
                                <x-buttons.primary type="submit" fullWidth="true" color="teal" icon="check_circle">
                        ...
                        @if($kelas->status !== 'selesai')
                            <form action="{{ route('admin.courses.updateStatus', $kelas->kelas_id) }}" method="POST">
                                ...
                                <x-buttons.primary type="submit" onclick="return confirm('Arsipkan kelas ini?')" fullWidth="true" color="gray" icon="inventory">
                                    Arsipkan Kelas
                                </x-buttons.primary>
                            </form>
                        @endif
                        ...
                        @if($kelas->status !== 'ditolak')
                            <form action="{{ route('admin.courses.updateStatus', $kelas->kelas_id) }}" method="POST">
                                ...
                                <x-buttons.primary type="submit" onclick="return confirm('Tolak kelas ini? Kelas tidak akan tampil di platform.')" fullWidth="true" color="red" icon="cancel">
                                    Reject/Tolak Kelas
                                </x-buttons.primary>
                            </form>
                        @endif

                        <form action="{{ route('admin.courses.destroy', $kelas->kelas_id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <x-buttons.secondary type="submit" onclick="return confirm('HAPUS PERMANEN kelas ini? Tindakan tidak bisa dibatalkan!')" fullWidth="true" color="red" icon="delete_forever">
                                Hapus Permanen
                            </x-buttons.secondary>
                        </form>
                    </div>
