        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            <form method="GET" action="{{ route('admin.pengajar.index') }}" class="flex flex-wrap gap-4">
                ...
                @if(request('search') || request('status') != 'all')
                    <x-buttons.secondary href="{{ route('admin.pengajar.index') }}" size="lg" icon="refresh" class="bg-gray-100 hover:bg-gray-200 text-gray-700 border-none">
                        Reset
                    </x-buttons.secondary>
                @endif
            </form>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <x-buttons.primary href="{{ route('admin.pengajar.show', $pengajar->user_id) }}" color="indigo" size="sm" aria-label="Lihat detail pengajar">
                                            Detail
                                        </x-buttons.primary>
                                        ...
                                        @if($pengajar->status === 'aktif')
                                            <form action="{{ route('admin.pengajar.updateStatus', $pengajar->user_id) }}"
                                                ...
                                                <x-buttons.primary type="submit" onclick="return confirm('Suspend pengajar ini?')" color="red" size="sm" aria-label="Suspend pengajar">
                                                    Suspend
                                                </x-buttons.primary>
                                            </form>
                                        @else
                                            <form action="{{ route('admin.pengajar.updateStatus', $pengajar->user_id) }}"
                                                ...
                                                <x-buttons.primary type="submit" color="teal" size="sm" aria-label="Aktifkan pengajar">
                                                    Aktifkan
                                                </x-buttons.primary>
                                            </form>
                                        @endif
