...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-md">
    ...
    <form action="{{ route('teacher.materi.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            @if($kelas->isEmpty())
                <p class="text-red-500 text-xs mt-1">Anda belum memiliki kelas aktif. <a href="{{ route('teacher.kelas.create') }}" class="underline font-bold">Buat kelas dulu</a>.</p>
            @endif
        </div>

        <!-- Judul Materi -->
        <div class="mb-6">
            <x-form.input name="judul" label="Judul Materi" placeholder="Contoh: Pengenalan Algoritma" required :value="old('judul')" />
        </div>
        ...
        <!-- Deskripsi -->
        <div class="mb-8">
            <x-form.textarea name="deskripsi" label="Deskripsi Tambahan (Opsional)" rows="4" placeholder="Berikan instruksi atau penjelasan singkat tentang materi ini...">{{ old('deskripsi') }}</x-form.textarea>
        </div>
        ...
        <div class="flex items-center justify-end space-x-4">
            ...
            <x-buttons.primary type="submit" class="px-6 py-2">
                <x-icons.material name="upload" size="sm" class="mr-2 -ml-1" />
                Simpan Materi
            </x-buttons.primary>
        </div>
    </form>
