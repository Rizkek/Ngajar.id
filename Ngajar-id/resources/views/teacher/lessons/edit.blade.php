...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-md">
    <div class="mb-6 border-b pb-4 flex justify-between items-center">
        ...
        <!-- Delete Button -->
        <form action="{{ route('teacher.materi.destroy', $materi->materi_id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus materi ini? File akan dihapus permanen.');">
            @csrf
            @method('DELETE')
    ...
    <form action="{{ route('teacher.materi.update', $materi->materi_id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        
        <!-- Pilih Kelas -->
        <div class="mb-6">
            <x-form.select name="kelas_id" label="Untuk Kelas" required>
                @foreach($kelas as $k)
        ...
        <!-- Judul Materi -->
        <div class="mb-6">
            <x-form.input name="judul" label="Judul Materi" placeholder="Contoh: Pengenalan Algoritma" required :value="old('judul', $materi->judul)" />
        </div>
        ...
        <!-- Deskripsi -->
        <div class="mb-8">
            <x-form.textarea name="deskripsi" label="Deskripsi Tambahan" rows="4" placeholder="Berikan instruksi atau penjelasan singkat...">{{ old('deskripsi', $materi->deskripsi) }}</x-form.textarea>
        </div>

        <!-- Buttons -->
        <div class="flex items-center justify-end space-x-4">
            <x-buttons.secondary href="{{ route('teacher.materi') }}" class="px-6 py-2">
                Batal
            </x-buttons.secondary>
